...
const Hero = ({ isScrolled }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute border-b-2 md:border-b-4 border-m3-primary left-0 top-0 w-[180px] sm:w-[200px] md:w-[300px] h-[50px]  md:h-[64px] xl:w-[420px] xl:h-[80px] bg-m3-primaryDark z-1 rounded-br-full transition-all duration-300 shadow-m3-lg ${isScrolled ? 'opacity-0' : 'opacity-100'}`}
      ></div>
      <img
        src={logo}
        className={`absolute rounded-full border-2 md:border-4 border-m3-primary z-2 transition-all duration-300 shadow-m3-lg top-4 left-12 h-[80px] w-[80px] md:h-[120px] md:w-[120px] lg:h-[180px] lg:w-[180px] xl:h-[240px] xl:w-[240px] ${isScrolled ? 'opacity-0' : 'opacity-100'}`}
        alt="Logo"
      />
